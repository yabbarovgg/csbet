...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const { user, logout, updateUser: updateUserData } = useAuth();
  ...
  return (
    <aside className={`${bg} border-l ${border} h-full lg:h-[calc(100vh-88px)] lg:sticky lg:top-[88px] flex flex-col overflow-hidden transition-colors duration-300 w-80`}>
      ...
      {cropImage && (
        <AvatarCrop
          imageSrc={cropImage}
          onCrop={(dataUrl) => { updateUserData({ avatar: dataUrl }); setCropImage(null); }}
          ...
          <div className="flex-1 min-w-0">
            {editingNick ? (
              <input
                ...
                className={`w-full border border-yellow-400/30 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:border-yellow-400/50 ${isDark ? 'bg-white/5 text-white' : 'bg-gray-100 text-gray-900 border-gray-300'}`}
                maxLength={30}
              />
            ) : (
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  className={`w-4 h-4 rounded shrink-0 transition-all cursor-pointer ${
                    showGradient
                      ? 'bg-gradient-to-r from-yellow-400 via-green-400 to-blue-400 opacity-60 hover:opacity-100'
                      : isDark ? 'bg-white/10 opacity-40 hover:opacity-70' : 'bg-gray-200 opacity-40 hover:opacity-70'
                  }`}
            ...
            <div className="flex items-center gap-2 mt-1.5">
              <span className={`text-xs font-mono font-semibold ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>ID: {user?.id}</span>
            </div>
      ...
      <div className={`flex border-b ${border}`}>
        <button onClick={() => setActiveTab('betSlip')}
          className={`flex-1 py-3 text-sm font-semibold transition-all relative ${activeTab === 'betSlip' ? textP : inactiveTab}`}>
          <span className="flex items-center justify-center gap-2">
            Купон
            {selections.length > 0 && <span className="bg-red-500 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center">{selections.length}</span>}
          </span>
          {activeTab === 'betSlip' && <div className="absolute bottom-0 left-4 right-4 h-0.5 bg-yellow-400 rounded-full" />}
        </button>
        <button onClick={() => setActiveTab('history')}
          className={`flex-1 py-3 text-sm font-semibold transition-all relative ${activeTab === 'history' ? textP : inactiveTab}`}>
          <span className="flex items-center justify-center gap-2">
            История
            {betHistory.length > 0 && <span className={`text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center ${isDark ? 'bg-white/10 text-gray-400' : 'bg-gray-200 text-gray-500'}`}>{betHistory.length}</span>}
          </span>
        ...
        {activeTab === 'betSlip' ? (
          <div className="p-4">
            ...
            <div className="bg-gradient-to-br from-yellow-400/10 to-amber-400/5 rounded-2xl p-4 border border-yellow-400/20 mb-4">
              ...
              <div className={`text-2xl font-black tabular-nums ${textP}`}>
                {formatBalance(balance)} <span className={`text-base ${textS}`}>₽</span>
              </div>
            </div>
            ...
            <div className="mb-4">
              <LoanBlock
                loan={activeLoan}
                onRepayFull={onRepayFull}
                onRepayPartial={onRepayPartial}
                isDark={isDark}
              />
            </div>
            ...
            {selections.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <div className={`w-16 h-16 ${isDark ? 'bg-white/5' : 'bg-gray-100'} rounded-2xl flex items-center justify-center mb-4`}>
              ...
              <>
                <div className="flex items-center justify-between mb-3">
                  <span className={`text-xs font-bold uppercase tracking-wider ${textS}`}>
                    {selections.length} {selections.length === 1 ? 'ставка' : selections.length < 5 ? 'ставки' : 'ставок'}
                  </span>
                ...
                <div className="flex flex-col gap-2 mb-4">
                  {selections.map((sel, idx) => (
                    <div key={`${sel.matchId}-${idx}`} className={`${bgC} rounded-xl p-3 border relative group`}>
                      <button onClick={() => onRemoveSelection(sel.matchId)}
                        className="absolute top-2 right-2 w-5 h-5 bg-white/5 rounded-full flex items-center justify-center text-gray-600 hover:text-red-400 hover:bg-red-400/10 transition-all opacity-0 group-hover:opacity-100 cursor-pointer">×</button>
                  ...
                  <div className="flex gap-1.5 mt-2">
                    {quickStakes.map((amount) => (
                      <button key={amount} onClick={() => setStake(amount.toString())}
                        className={`flex-1 py-1.5 rounded-lg text-xs font-medium transition-all border cursor-pointer ${
                          stake === amount.toString()
                            ? 'bg-yellow-400/20 text-yellow-400 border-yellow-400/30'
                            : `${isDark ? 'bg-white/5 border-white/5 text-gray-500 hover:text-white hover:bg-white/10' : 'bg-gray-100 border-gray-200 text-gray-500 hover:text-gray-900 hover:bg-gray-200'}`
                        }`}>
                    ...
                    <button onClick={() => setStake(balance.toFixed(2))}
                      className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all border cursor-pointer ${
                        stake === balance.toFixed(2)
                          ? 'bg-red-400/20 text-red-400 border-red-400/30'
                          : `${isDark ? 'bg-white/5 border-white/5 text-red-400/80 hover:text-red-400 hover:bg-red-400/10 hover:border-red-400/20' : 'bg-gray-100 border-gray-200 text-red-400/80 hover:text-red-400 hover:bg-red-400/10 hover:border-red-400/20'}`
                      }`} title="Ставка на весь баланс">
                      MAX
                    </button>
                  </div>
                </div>
                ...
                <button onClick={handlePlaceBet}
                  disabled={stakeNum <= 0 || stakeNum > balance + 0.001 || selections.length === 0 || isPlacing}
                  className={`w-full py-3.5 rounded-xl font-bold text-sm transition-all duration-200 cursor-pointer ${
                    stakeNum > 0 && stakeNum <= balance + 0.001 && selections.length > 0 && !isPlacing
                      ? 'bg-gradient-to-r from-yellow-400 to-amber-400 text-black hover:opacity-90 active:scale-[0.98]'
                      : `${isDark ? 'bg-white/5 text-gray-600' : 'bg-gray-100 text-gray-400'} cursor-not-allowed`
                  }`}>
                  {isPlacing ? (
                    <span className="flex items-center justify-center gap-2">
                      <div className="w-4 h-4 border-2 border-black/30 border-t-black rounded-full animate-spin" />Оформление...
                    </span>
                  ) : 'Сделать ставку'}
                </button>
              </>
      ...
      <div className={`p-4 border-t ${border}`}>
        <button onClick={logout}
          className={`w-full py-2.5 rounded-xl text-sm font-medium transition-all border cursor-pointer ${
            isDark ? 'border-white/5 text-gray-500 hover:text-red-400 hover:border-red-400/20 hover:bg-red-400/5' : 'border-gray-200 text-gray-500 hover:text-red-500 hover:border-red-400/30 hover:bg-red-50'
          }`}>
          Выйти из аккаунта
        </button>
      </div>
